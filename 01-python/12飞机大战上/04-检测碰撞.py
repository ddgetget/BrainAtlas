...
# 加载英雄飞机的图片对象
hero_img = pygame.image.load("/mnt/hgfs/WorkSpace/data/planeBattle/res/hero2.png")
# 获取飞机的矩形对象
hero_img_rect = hero_img.get_rect()
print("英雄:", hero_img_rect)

hero_img_rect[0] = 512 / 2 - hero_img_rect[2] / 2
hero_img_rect[1] = 150
print("英雄:", hero_img_rect)
...
for i in range(10):
    ...
    # 设置英雄飞机显示的位置就是矩形对象的位置
    window.blit(hero_img, (hero_img_rect[0], hero_img_rect[1]))
    # 刷新窗口
    pygame.display.update()
    time.sleep(0.2)

    # 判断飞机是否碰撞，其实检测额是敌机的矩形对象和英雄飞机的矩形对象是否有重叠
    # 有重叠表示碰撞，否则没有碰撞
    if pygame.Rect.colliderect(enemy_img_rect, hero_img_rect):
        print("碰撞啦")
    else:
        print("没有碰撞")
